...
//  AnimeList
//
//  Created by Alexandra Charland on 2/15/21.
//
...
class SearchViewController: UITableViewController, UISearchResultsUpdating {
    ...
    func updateSearchResults(for searchController: UISearchController) {
        let searchString = searchController.searchBar.text //search string
                searchTitles.removeAll(keepingCapacity: true) //removes all elements
                if searchString?.isEmpty == false {
                    ...
                    let matches = allTitles.filter(searchfilter)
                    searchTitles.append(contentsOf: matches)
                }
